...
def extract(pdf_path):
    ...
    # --- header facts from page 2 (index 1) and page 1 (index 0)
    ward = gram = samiti = booth = ""
    pages_to_check = [1, 0] if len(doc) > 1 else [0]
    for pno in pages_to_check:
        ...
        for k in sorted(rows, reverse=True):
            ...
            if not samiti:
                ...
                if m_ps:
                    samiti = m_ps.group(1).strip()
            if not gram:
                m_gp = re.search(r"(?:[गध]?\s*्राम\s*पंचायत|ाम\s*पंचायत|ग्राम\s*पंचायत)\s*:\s*(.+?)(?:वार्ड|क्रमांक|$)", line)
                if m_gp:
                    ...
                    gram = val
            if not ward:
                m_w = re.search(r"(?:वार्ड\s*क्रमांक|क्रमांक)\s*:\s*(\d+)", line)
                if m_w:
                    ward = m_w.group(1).strip()
            if not booth:
                if "मतदान" in line and "पता" in line and ":" in line:
    ...
    if not gram:
        if "GHORAWAR" in fn_upper or "GHODAWAD" in fn_upper: gram = "घोड़ावड़"
        elif "BASSI" in fn_upper: gram = "बस्सी"
        elif "BALOONDA" in fn_upper: gram = "बलून्दा"
        elif "BADAKHERA" in fn_upper: gram = "बड़ाखेड़ा (खेड़ा कलां)"
        elif "JAWAJA" in fn_upper: gram = "जवाजा"
    if not samiti:
        if any(w in fn_upper for w in ["GHORAWAR", "GHODAWAD", "BASSI", "BALOONDA"]):
            samiti = "जैतारण"
        elif any(w in fn_upper for w in ["BADAKHERA", "JAWAJA"]):
            samiti = "जवाजा"
